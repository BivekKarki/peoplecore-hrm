...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    if (check_in && check_out) {
      const diff = new Date(check_out).getTime() - new Date(check_in).getTime();
    ...
    const res = await query<AttendanceRecord>(
      `INSERT INTO attendance (employee_id, date, check_in, check_out, hours_worked, status, method, notes)
       ...
       RETURNING *`,
      [employee_id, date, check_in || null, check_out || null, hoursWorked, status, method || 'manual', notes || null]
    );
